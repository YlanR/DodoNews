...
<?php if($total > 0){?>
    ...
    <?php foreach($tecnologia as $tecno): ?>
        <div class="block">
            <article id="tresbloques">
                <h2><a href="<?=base_url()?>/contenido/<?=$tecno["id_publicacion"] ?>"><?=$tecno["titulo"] ?></a></h2>
                
                <figure>
                    <a href="<?=base_url()?>/contenido/<?=$tecno["id_publicacion"] ?>"><img src="<?=base_url()?>/uploads/<?=$tecno["imagen"] ?>"></a>
                    <figcaption>
                       <span>Por <?=$tecno["usuario"] ?></span>
                    </figcaption>
                </figure>                       
            </article>
        </div>
