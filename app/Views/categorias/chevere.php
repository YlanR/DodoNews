...
<?php if($total > 0){?>
    ...
    <?php foreach($chevere as $cheve): ?>
        <div class="block">
            <article id="tresbloques">
                <h2><a href="<?=base_url()?>/contenido/<?=$cheve["id_publicacion"] ?>"><?=$cheve["titulo"] ?></a></h2>
                
                <figure>
                    <a href="<?=base_url()?>/contenido/<?=$cheve["id_publicacion"] ?>"><img src="<?=base_url()?>/uploads/<?=$cheve["imagen"] ?>"></a>
                    <figcaption>
                       <span>Por <?=$cheve["usuario"] ?></span>
                    </figcaption>
                </figure>                       
            </article>
        </div>
